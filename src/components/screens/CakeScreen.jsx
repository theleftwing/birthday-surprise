...
export default function CakeScreen({ onNext }) {
  ...
  return (
    <div className="bg-[#fff8fc] p-7 rounded-[60px] drop-shadow-2xl w-full max-w-[440px] relative flex flex-col items-center gap-6 my-10">
      
      <motion.div
        className="text-center text-3xl md:text-4xl font-semibold text-secondary leading-tight px-4"
      ...
      <div className="relative flex flex-col items-center gap-8 w-full">
        <div className="relative h-72 bg-gradient-to-b from-white/80 to-rose-200 w-full flex items-end justify-center rounded-[40px] shadow-inner pb-10">
          <Cake lit={lit} />
        </div>

        <AnimatePresence mode="wait">
          {!lit ? (
            <motion.div key="light" exit={{ opacity: 0, scale: 0.8 }}>
